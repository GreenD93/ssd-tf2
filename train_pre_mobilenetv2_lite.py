...
with tf.device('/device:GPU:1'):
    ...
    ssd = create_pre_ssd_mobilenetv2_lite(weights=None)
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        for i, (_, imgs, gt_confs, gt_locs) in enumerate(val_generator):
            ...
            if (i + 1) % 50 == 0:
                print('Val_Epoch: {} Batch {} Time: {:.2}s | Val_Loss: {:.4f} Val_Conf: {:.4f} Val_Loc: {:.4f}'.format(
                    epoch + 1, i + 1, time.time() - start, avg_val_loss, avg_val_conf_loss, avg_val_loc_loss))
        ...
        if (epoch + 1) % 100 == 0:
            ssd.save_weights(
                os.path.join(CHECKPOINT_DIR, 'ssd_epoch_{}.h5'.format(epoch + 201)))
            end = time.time()
            print('>>>>>>>> mobilenet_v2_lite: 100 epoch time : {0}'.format((end-train_start)/train_num))
            train_num += 1