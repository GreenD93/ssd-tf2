...
def create_pre_ssd_mobilenetv1_lite(weights='imagenet'):
    
    num_classes = NUM_CLASSES
    arch = 'pre_ssd300-mobilenetv1_lite'
...
def base_net(weights):
    ...
    if weights == 'imagenet':
        base_model = MobileNet(include_top=False, weights='imagenet', input_shape=(300, 300, 3))
    else:
        base_model = MobileNet(include_top=False, weights=None, input_shape=(300, 300, 3))
    
    # 4th block
    # (None, 300, 300, 3) -> (None, 18, 18, 512)
    # Up to conv_pw_11_relu (ReLU)
    mobile_v1_conv4 = []
